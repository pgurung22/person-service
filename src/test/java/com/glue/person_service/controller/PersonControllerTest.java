...
class PersonControllerTest {
    ...
    void getPeopleByName_shouldReturnPeople() {
        ...
        when(personServiceImpl.getPeopleByNameOrAge(eq("John"), eq(30), any(PageRequest.class))).thenReturn(people);
    ...
    void getPeopleByName_shouldReturnNotFound() {
        when(personServiceImpl.getPeopleByNameOrAge(eq("John"), eq(30), any(PageRequest.class))).thenReturn(Collections.emptyList());
        ResponseEntity<?> response = personController.getPeopleByName(null, null, "John", 30);
    ...
    void updatePersonById_shouldReturnCreated() {
        when(personServiceImpl.updatePersonById(eq(1L), any(PersonDTO.class))).thenReturn(new PersonDTO());
        ResponseEntity<String> response = personController.updatePersonById(1L, new PersonDTO());
        assertEquals(HttpStatus.CREATED, response.getStatusCode());
